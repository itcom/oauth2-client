...
class Au extends AbstractProvider
{
    ...
    public function getAuthorizationUrlCp($options = [])
    {
        $this->state = isset($options['state']) ? $options['state'] : md5(uniqid(rand(), true));
        $_SESSION ['oauth2state'] = $this->state;

        $params = [
            'client_id' => \Config::get('site.accounting.ausmpass.clientId', null),
            'redirect_uri' => \Config::get('site.accounting.ausmpass.redirectUrl', null),
            'state' => $this->state,
            'scope' => is_array($this->scopes) ? implode($this->scopeSeparator, $this->scopes) : $this->scopes,
            'response_type' => isset($options['response_type']) ? $options['response_type'] : 'code',
            'approval_prompt' => isset($options['approval_prompt']) ? $options['approval_prompt'] : 'auto',
    ...
    public function userDetailsCp($response, AccessToken $token)
    {
        try {
        ...
        $client->setBaseUrl($this->urlUserDetails($token));
        $requestParams = ['ver' => '1.0'];
        $this->headers = ['x-sr-id' => \Config::get('site.accounting.ausmpass.srId', null)];
        $headers = $this->getHeaders($token->accessToken);
        logger(\Fuel::L_DEBUG, var_export( $headers, true), __METHOD__);
        $response = $client->post(null
            , $headers
            , $requestParams
        )->send();
        logger(\Fuel::L_DEBUG, var_export( $response->getBody()->__toString(), true), __METHOD__);
        $result = $this->prepareResponse($response);
        } catch (BadResponseException $e) {
            // @codeCoverageIgnoreStart
            $response = $e->getResponse();
    ...
    public function getAccessTokenCp($grant = 'authorization_code', $params = [])
    {
        if (is_string($grant)) {
            // PascalCase the grant. E.g: 'authorization_code' becomes 'AuthorizationCode'
            $className = str_replace(' ', '', ucwords(str_replace(['-', '_'], ' ', $grant)));
        ...
        $defaultParams = [
            ...
            'grant_type'    => $grant,
            'client_id' => \Config::get('site.accounting.ausmpass.clientId', null),
            'client_secret' => \Config::get('site.accounting.ausmpass.clientSecret', null),
        ];
